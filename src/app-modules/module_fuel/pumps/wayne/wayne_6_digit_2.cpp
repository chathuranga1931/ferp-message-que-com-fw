...
// Logic is identical to wayne_6_digit_1.cpp — the only difference is the
// unit-price sanity check in wayne62_process_data(): this pump is
// deliberately configured with a fixed unit price of 1.00 for a specific
// reason (not a real per-liter price), so the shared driver's >=200.00
// floor (calibrated for realistic fuel prices) would reject every frame.
// Here only a genuinely absent/zero unit price is rejected.
...
#define __TAG__  "APP_WN62"

#define WNE62_DEBUG_LOG_EN      false
#define WNE62_WARN_LOG_EN       true
#define WNE62_ERROR_LOG_EN      true
#define WNE62_INFO_LOG_EN       true
...
static bool wayne62_commit_data(uint8_t idx){
...
static void wayne62_update_data(uint8_t idx){
...
bool wayne62_process_data(app_display_data_t * display_data){
    ...
    do{
        // Unlike wayne6_process_data(), this pump is deliberately
        // configured with a fixed unit price of 1.00 for a specific
        // reason, not a real per-liter price — so only reject a
        // genuinely absent/zero unit price, not a low one.
        if(display_data->unit_pricex100 == 0){
...
bool wayne62_process_state_machine(const app_display_data_t * display_data, uint8_t nozzle_id, pumping_state_t * state){
    ...
    switch (pumping_state_value_based[nozzle_id])
    {
        case Pumping_State_Pumping:
            if(deravative_int == 0){
                if((ts - pumping_state_pumping_ts[nozzle_id]) > 500){
                    ...
                    LOG_MSG_INFO(WNE62_INFO_LOG_EN, "[Value] State [%d] : Pumping --> Pumping-Waiting", nozzle_id);
                }
            }
            else if(deravative_int == 1){
        ...
        case Pumping_State_Pumping_Waiting:
            if(deravative_int == 1){
                ...
                pumping_state_ts[nozzle_id] = ts;
                pumping_state_pumping_ts[nozzle_id] = ts;
                LOG_MSG_INFO(WNE62_INFO_LOG_EN, "[Value] State [%d] : Pumping-Waiting --> Pumping ", nozzle_id);
            }
        break;
        ...
        default:
            if(deravative_int == 1){
                pumping_state_value_based[nozzle_id] = Pumping_State_Pumping;
                pumping_state_ts[nozzle_id] = ts;
                LOG_MSG_INFO(WNE62_INFO_LOG_EN, "[Value] State [%d] : Unknown --> Pumping ", nozzle_id);
            }
        break;
    }
    ...
    switch(pumping_state_nozzle_based[nozzle_id])
    {
        case Pumping_State_Pumping:
            if(!(display_data->start_stop))
            {
                ...
                )
                {
                    ...
                    LOG_MSG_INFO(WNE62_INFO_LOG_EN, "[Nozzle] State [%d] : Pumping --> Unknown ", nozzle_id);
                }
                else
                {
                    pumping_state_nozzle_based[nozzle_id] = Pumping_State_Stopped_Waiting;
                    LOG_MSG_INFO(WNE62_INFO_LOG_EN, "[Nozzle] State [%d] : Pumping --> Stopped-Waiting ", nozzle_id);
                    pumping_state_nozzle_ts[nozzle_id] = ts;
                    wayne62_commit_data(nozzle_id);
                }
            }
        break;
        ...
        case Pumping_State_Stopped:
            ...
            pumping_state_nozzle_based[nozzle_id] = Pumping_State_Unknown;
            pumping_state_nozzle_ts[nozzle_id] = ts;
            LOG_MSG_INFO(WNE62_INFO_LOG_EN, "[Nozzle] State [%d] : Stopped --> Unknown ", nozzle_id);
            break;
        ...
        default:
            if(display_data->start_stop)
            {
                ...
                pumping_state_nozzle_ts[nozzle_id] = ts;
                LOG_MSG_INFO(WNE62_INFO_LOG_EN, "[Nozzle] State [%d] : Unknown --> Pumping ", nozzle_id);
            }
        break;
    }
