...
typedef enum : uint16_t
{
    ...
    // Typed domain config requests (sent after MsgConfigReady) ─────────
    ...
    // Typed domain config responses (DIRECT -> requester) ──────────────
    MSG_ID_CONFIG_WIFI          = 0x0307,   ///< ModuleConfig -> ModuleWifi:  WiFi credentials
    MSG_ID_CONFIG_CLOUD         = 0x0308,   ///< ModuleConfig -> ModuleCloud: cloud endpoint + identity
    MSG_ID_CONFIG_MQTT          = 0x0309,   ///< ModuleConfig -> ModuleMqtt:  MQTT broker settings
    MSG_ID_CONFIG_DT            = 0x030A,   ///< ModuleConfig -> ModuleFuel:  display type + hardware
