...
// cube_sphere C API.
//
// This file is ONLY compiled for device builds (ESP-IDF / Arduino).
// It must NOT be included in the simulator CMakeLists — the simulator injects
// a null driver instead.
...
{
    // cube_sphere_register takes a non-const pointer but only reads it
...
static startup_info_t _build_startup_info(cloud_startup_info_t info)
{
    startup_info_t s = {};
...
{
    return cube_sphere_send_startup(_build_startup_info(info));
}

static int32_t _send_status_updated(cloud_startup_info_t info)
{
    return cube_sphere_send_status_updated(_build_startup_info(info));
}
