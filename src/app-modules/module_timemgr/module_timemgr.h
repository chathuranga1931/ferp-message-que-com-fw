...
//
// Responsibilities:
//   1. On boot: try DS1307 RTC immediately, then load NVS backup.
//   2. On internet connection: synchronise via NTP (highest priority source).
//   3. Every 5 minutes: save a backup of the current epoch to NVS.
//   4. Publish MsgTimeStatus whenever the time source changes.
//
// Source priority (lowest → highest):
//   NONE → BACKUP (NVS) → RTC (DS1307) → NTP
