// msg_mqtt_status.h
//
// Published by ModuleMqtt when the MQTT broker connection state changes.
// Subscribers (e.g. ModuleSimBridge) use this to update status indicators.
...
class MsgMqttStatus : public IHsysMsg
{
public:
    static constexpr hsys_msg_id_t ID = MSG_ID_MQTT_STATUS;
    ...
    explicit MsgMqttStatus(const Payload &p) : _p(p) {}
