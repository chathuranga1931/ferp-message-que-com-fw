...
// mbedtls 4.x (IDF 6.0) removed the per-algorithm headers; use md.h for hashes.
                          ...
                          uint8_t* output)
{
    ...
    if (!info) {
        LOG_MSG_ERROR(CRYP_DEBUG_LOG_EN, "SHA256 not available");
        return -1;
    }

    int ret = mbedtls_md(info, input, input_len, output);
    if (ret != 0) {
        LOG_MSG_ERROR(CRYP_DEBUG_LOG_EN, "SHA256 failed");
        return -1;
    }
    return 0;
}
                       ...
                       uint8_t* output)
{
    ...
    if (!info) {
        LOG_MSG_ERROR(CRYP_DEBUG_LOG_EN, "MD5 not available");
        return -1;
    }

    int ret = mbedtls_md(info, input, input_len, output);
    if (ret != 0) {
        LOG_MSG_ERROR(CRYP_DEBUG_LOG_EN, "MD5 failed");
        return -1;
    }
    return 0;
}
