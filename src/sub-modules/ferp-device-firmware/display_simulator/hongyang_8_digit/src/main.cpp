...
const size_t repeat_packet = 2;
const size_t repeat_packet_ll = 2;
...
void send_byte(uint8_t byte1, uint8_t byte2, bool skip_rclk_low);
...
void create_buffer_hongyang_8(float volume, float price)
{
    ...
    for (int i = IDX_LITERS_0; i >= IDX_LITERS_START; i--)
    {
        if (volume_0001)
        {
            sdata2_tx[i].DIG = volume_0001 % 10;
            volume_0001 /= 10;
        }
        ...
        {
            sdata2_tx[i].DIG = DIS_CHARACTOR_BLANK; // blank for the rest
        }
    }
}
...
void create_buffer_hongyang_8_dummy_ll()
{
    ...
    sdata1_tx[8].DIG = 0x1;
    sdata1_tx[9].DIG = 0x8;
    sdata1_tx[10].DIG = 0x3;
    sdata1_tx[11].DIG = 0x3;
    sdata1_tx[12].DIG = 0x1;
    sdata1_tx[13].DIG = 0x3;
    ...
    sdata2_tx[8].DIG = 0xF;
    sdata2_tx[9].DIG = 0xF;
    sdata2_tx[10].DIG = 0xF;
    sdata2_tx[11].DIG = 0x1;
    sdata2_tx[12].DIG = 0xF;
    sdata2_tx[13].DIG = 0xF;
}
    ...
    while (1)
    {
        ...
        for (size_t j = 0; j < repeat_packet; j++)
        {
            ...
            for (size_t k = 0; k < PACKET_SIZE; k++)
            {
                ...
                if(k == PACKET_DELAY_IDX) //add 7ms delay at end of packet ID 7
                {
                    send_byte(sdata1_tx[k].u8int, sdata2_tx[k].u8int, true);
                    delayMicroseconds(6665);
                }
                else
                {
                    send_byte(sdata1_tx[k].u8int, sdata2_tx[k].u8int, false);
                }
            }
            digitalWrite(c_pin_green_led, LOW);
            delay(500);
...
totaliser:
    ...
    for (size_t k = 0; k < PACKET_SIZE; k++)
    {
        ...
        else if (k == PACKET_SKIP_RCLK_LOW_ID)//at the end of the packet, keep RCLK high
        {
            send_byte(sdata1_tx[k].u8int, sdata2_tx[k].u8int, true);
        }
        else
    ...
    for (size_t k = 0; k < PACKET_SIZE; k++)
    {
        ...
        else if(k == PACKET_DELAY_IDX_LL)//add 348us delay at end packet ID 0xA
        {
            send_byte(sdata1_tx[k].u8int, sdata2_tx[k].u8int, true);
...
void send_packet(uint32_t int_delay)
{
