...
echo "Building..."
pio run -e esp32dev-printer
pio run -t buildfs -e esp32dev-printer

echo "Creating FERP-IoT-Printer Release Package"

FW_VERSION_STRING=$(cat ./src/Application/version.h | grep "#define.*FW_VERSION_PRINTER" | head -1)
#output should like below
#define FW_VERSION "1.0.5"
FW_VERSION_STR2=$(echo $FW_VERSION_STRING | sed -e "s/#define FW_VERSION_PRINTER //g")
...
if [[ $1 == "--ignore-tag" ]]; then
	...
	is_tagged=$(git tag | grep "PTR-V$FW_VERSION")
	if [[ ! -z $is_tagged ]]; then
		echo "Version is already Tagged, Update it before proceeding"
		exit 1
	fi

	git tag "PTR-V$FW_VERSION"
	git push origin "PTR-V$FW_VERSION"
fi

FOLDER_NAME="FERP_IoT_Printer_Release_"$FILE_VERSION
echo $FOLDER_NAME

BUILD_FOLDER="./.pio/build/esp32dev-printer"
