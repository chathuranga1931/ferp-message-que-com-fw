...
typedef struct __attribute__((packed))
{
    ...
    uint8_t sw_minor;
    uint8_t display_id;
    display_data_v2_t display_data;
    uint32_t checksum;
} packet_data_t;
...
void display_tap()
{
            ...
            if (dt_rxdata_index > 4) // got the packet
            {
                ...
                if (checksum == rx_data.checksum){
                    ...
                    if (rx_data.display_id < CB_SIZE)
                    {
                        ...
                        logger.log("Got dummy packet");
                    }
                    else{
                        logger.log("Invalid ID:" + String(rx_data.display_id));
                    }
                }
                else{
                    logger.log("Serial data error ");
                }
            }
            else
            {
    ...
    {
        time_display_rec = time_now;
        logger.log("Display Tap Offline. Resetting...");
        digitalWrite(GPIO_N_RESET_ESP07, LOW); // starting reset
    ...
    if (reset_display_tap)
    {
        if ((time_now - time_reset) > reset_time_ms)
        {
            ...
            digitalWrite(GPIO_N_RESET_ESP07, HIGH); // reset done
            logger.log("DT reset done...");
        }
    }
}
