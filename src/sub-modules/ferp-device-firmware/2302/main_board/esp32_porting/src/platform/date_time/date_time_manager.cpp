...
extern ret_t load_last_working_time(time_t * time);
ret_t date_time_sync();
...
static ret_t update_time_from_ntp(time_t * ephoc_time){
    ...
    do{
        ...
        _device_configs->status.date_time_status = date_time_syced;
        while(!timeClient.update()) {
            if(millis() - ts > (_device_configs->datetime.wait_time_s * 1000)){
                _device_configs->status.date_time_status = date_time_sync_failed;
                break;
            }
        ...
        if(_device_configs->status.date_time_status == date_time_sync_failed){
...
String get_date_as_string(long tv_sec){
    String full_date_time = timeClient.getFormattedDate(tv_sec, _device_configs->datetime.region_str);
    full_date_time.remove(full_date_time.indexOf('T'));
    logger.log(full_date_time);
...
void initialize_rtc(int retry_count){
...
ret_t date_time_init(device_configs_t * device_configs){
    ...
    if(_device_configs->status.date_time_status != date_time_syced){

        logger.log(F("TS: Date time updated from last working time... "));
        time_t time;
        load_last_working_time(&time);
    ...
    if(_device_configs->status.rtc_status != rtc_available) {
        ...
        /* if WiFi Not available */
            ...
            if(_device_configs->status.rtc_status == rtc_available){
                ...
                // use default time
                ret = ret_Err_App_Failed_to_sync_date_time;
                _device_configs->status.date_time_status = date_time_sync_failed;
                logger.log(F("TS: Err!! No source to sync the time is available..."));
            }
            #endif
        }
