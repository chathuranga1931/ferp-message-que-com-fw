...
static xTaskHandle ser_rx_task_hdl = NULL;
...
bool esp07_send_cmd(data_packet_t *rx, data_packet_t *tx, uint32_t tout)
{
    ...
    while (!got_data)
    {
        ...
        {
            rx_packet = NULL; //clear store packet
            return false;
        }
...
static void serial_receive_task(void *arg)
{
    ...
    while (1)
    {
        len = uart_read_bytes(UART_NUM_2, (uint8_t *)(rx_buffer + index), (UART0_BUFF - index), pdMS_TO_TICKS(1));
        if (len)
        {
        ...
        {
            printf("No reply, resetting esp07\r\n");
            gpio_set_reset_esp07(true);
...
void suspend_comms_esp07()
{
    ...
    suspend_task = true;
    // if (ser_rx_task_hdl)
    //     vTaskSuspend(ser_rx_task_hdl);
    /*wait until task suspend*/
    printf("suspending esp07 comm task\r\n");
    vTaskDelay(pdMS_TO_TICKS(1 * 1000));
    // do
    // {
    //     vTaskGetTaskInfo(ser_rx_task_hdl, &pxTaskStatus, xGetFreeStackSpace, eState);
    //     vTaskDelay(pdMS_TO_TICKS(10));
    // } while (eState != eSuspended);
}
...
void resume_comms_esp07()
{
    ...
    printf("resuming esp07 comm task\r\n");
    // do
    // {
    //     vTaskGetTaskInfo(ser_rx_task_hdl, &pxTaskStatus, xGetFreeStackSpace, eState);
    //     vTaskDelay(pdMS_TO_TICKS(10));
    // } while (eState != eRunning);
}
