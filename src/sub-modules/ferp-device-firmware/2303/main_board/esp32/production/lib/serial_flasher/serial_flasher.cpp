...
#define ESP07_BASE_PATH "/spiffs/esp07/"
#define ESP07_BOOTLOADER_NAME "bootloader.bin"
#define BOOTLOADER_ADDRESS 0x00
#define ESP07_APPLICATION_NAME "rtos_dis_tap_esp07.bin"
#define APPLICATION_ADDRESS 0x10000
#define ESP07_PARTITION_TABLE "partitions_table.bin"
#define PARTITION_ADDRESS 0x8000

esp_err_t read_app_info(const char *file_name, esp_app_desc_t *app_desc);

void start_serial_flash()
{
    example_binaries_t bin = {
        .boot = {
            .file_name = ESP07_BASE_PATH ESP07_BOOTLOADER_NAME,
            .addr = BOOTLOADER_ADDRESS,
        },
        .part = {
            .file_name = ESP07_BASE_PATH ESP07_PARTITION_TABLE,
            .addr = PARTITION_ADDRESS,
        },
        .app = {
            .file_name = ESP07_BASE_PATH ESP07_APPLICATION_NAME,
    ...
    printf("app name:%s, ver:%s\r\n", app_desc.project_name, app_desc.version);
    err1 = esp07_get_fw_version(version);
    if (err1 == ESP_OK)
    {
        if (strcmp(app_desc.version, version) == 0)
        {
            printf("app version are matching, no firmware update\r\n");
            return;
        }
    }
    else
    {
        printf("esp07 version read failed.\r\n");
    }
    suspend_comms_esp07();

    printf("Starting Write Firmware...\r\n");

    gpio_set_mode_output_io0_esp07();
    const loader_esp32_config_t config = {
        .uart_port = UART_NUM_2,
        .reset_trg_fn = gpio_set_reset_esp07,
        .gpio0_trg_fn = gpio_set_io0_esp07};
