...
static xQueueHandle send_queue = NULL;
uint8_t rx_buffer[UART0_BUFF];
uint8_t tx_buffer[UART0_BUFF];

void send_response(data_packet_t *packet);
uint16_t dev_crc16(uint16_t wcrc, const void *kpvData, uint32_t length);
size_t make_tx_buffer(data_packet_t *packet, uint8_t *buffer)
{
    size_t idx = 0;
    buffer[idx++] = 0xFF;
    buffer[idx++] = 0xFF;                             // adding 2 0XFF s to the begning of the packet
    for (size_t i = 0; i < (packet->length + 4); i++) // adding 4 to packet length to add address + length + crcH + crcL
    {
...
bool validate_packet(data_packet_t *packet, int *idx)
{
    ...
    // calc total length
    const size_t b = packet->length + 4; //+4 for packet id + data lenght + CRCLSB + CRCMSB
    if (b != *idx)
    {
        return false;
    }
...
void serial_receive_task(void *arg)
{
    ...
    while (1)
    {
        len = uart_read_bytes(UART_NUM_0, (uint8_t *)(rx_buffer + index), (UART0_BUFF - index), pdMS_TO_TICKS(10));
        if (len)
        {
            for (size_t i = 0; i < len; i++)
            {
                ...
                {
                    // yes, is this the SOM?
                    if ((rx_buffer + index)[i] == 0xFF)
                        rx_idx = 0; // yes, so flag we're now storing bytes
                    continue;
                }
                else if ((rx_buffer + index)[i] == 0xFF) // EOM received?
                {
                    rx_doesc = false;
                ...
                if ((rx_buffer + index)[i] == 0xFE) // escaped char marker
                {
                ...
                {
                    // yes, so do it
                    (rx_buffer + index)[i] = ~((rx_buffer + index)[i]);
                    rx_doesc = false; // clear the flag
                }

                // index within range?
                if (rx_idx < (int8_t)sizeof(data_packet_t))
                {
                    // yes, so save the data
                    packet.ab_raw[rx_idx++] = (rx_buffer + index)[i];
                }
                else
                {
...
void serial_send_task(void *arg)
{
    ...
    while (1)
    {
        if (xQueueReceive(send_queue, &display_data, pdMS_TO_TICKS(10000))) // wait 10 seconds and send previous message
        {
            ...
            default:
                printf("Display not defined:%d", display_data.display);
                break;
            }
        #else
            const size_t len = display_data.length + 2;
...
esp_err_t device_init()
{
    ...
    ESP_ERROR_RETURN(uart_param_config(UART_NUM_0, &uart_config_dev));
    ESP_ERROR_RETURN(uart_driver_install(UART_NUM_0, UART0_BUFF, UART0_BUFF, 0, NULL, 0));
    ...
    case DIS_NONE: // during production
        ret = production_io_init(&send_queue);
        break;
    case DIS_CENSTAR_6_DIGIT:
    ...
    case DIS_CENSTAR_8_DIGIT:
        ret = display_censtar_8_digit_init(&send_queue);
        break;
    case DIS_HONGYANG_8_DIGIT:
