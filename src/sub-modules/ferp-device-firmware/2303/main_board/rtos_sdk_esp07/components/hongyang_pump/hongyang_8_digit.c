...
#define c_pin_sdata2_read_dis_1 (bool)(READ_PERI_REG(RTC_GPIO_IN_DATA) & 0x1) // because pin number is 16
...
#define c_pin_sdata2_read_dis_2 (bool)((GPIO.in & BIT(D2_SDATA2))) //((GPIO.in >> D2_SDATA2) & 0x1) // pin number 0
...
*
*/
static void get_display_data(hya_8_digit_t *dd, tu_byte_t *ab, tu_byte_t *bb)
{
	hya_8_digit_t data = {};
	...
	data.flags.select_p = (bool)(bb[IDX_SELECT_P].MS == DIS_CHARACTOR_P);

	data.flags.start_stop = (bool)(bb[IDX_START_STOP].MS == DIS_CHARACTOR_1);
	...
	memcpy((void *)dd, (void *)&data, sizeof(hya_8_digit_t));
}
	...
	while (1)
	{
		if (xQueueReceive(capture_queue, &capture_now, pdMS_TO_TICKS(10)))
		{
			...
			if (ticks_now - ticks_last[dis_id] > pdMS_TO_TICKS(DIFF_PCKT_SEND_MS) || memcmp_dataset(capture_dis + dis_id, &capture_now))
			{
				...
				get_display_data((hya_8_digit_t *)(display_data.ab_data), capture_now.ab, capture_now.bb);
				if ((((hya_8_digit_t *)(display_data.ab_data))->error.u8int & settings.error_mask.u8int) == 0) // if retain errors are none zero, do not send
				{
				...
				// const hya_8_digit_t *dis = (hya_8_digit_t *)(display_data.ab_data);
		...
		if ((xTaskGetTickCount() - ticks_last[TX_ID_DIS1_DATA]) > pdMS_TO_TICKS(SAME_PCKT_SEND_MS))
		{
		    ...
		    get_display_data((hya_8_digit_t *)(display_data.ab_data), temp.ab, temp.bb);
...
esp_err_t display_hongyang_8_digit_init(xQueueHandle *send_que)
{
	...
	io_conf.intr_type = GPIO_INTR_DISABLE;
	io_conf.mode = GPIO_MODE_INPUT;
	io_conf.pin_bit_mask = BIT(D1_SDATA1) | BIT(D1_SDATA2) | BIT(D2_SDATA1) | BIT(D2_SDATA2);
	io_conf.pull_down_en = GPIO_PULLDOWN_DISABLE;
	io_conf.pull_up_en = GPIO_PULLUP_DISABLE;
	gpio_config(&io_conf);
	...
	// printf("Staring Hongyang 8 display\r\n");
	return ESP_OK;
}
