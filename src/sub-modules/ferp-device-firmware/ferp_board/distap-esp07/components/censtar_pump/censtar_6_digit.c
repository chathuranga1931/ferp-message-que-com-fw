...
static void get_display_data(cens_6_digit_t *dd, tuByte_t *ab)
{
    cens_6_digit_t data = {};

    data.flags.select_ll = (bool)(ab[LL_IDX_LL_1].MS == DIS_CHARACTOR_L && ab[LL_IDX_LL_2].MS == DIS_CHARACTOR_L);
    data.flags.select_l = (bool)(ab[IDX_SELECT_L].MS == DIS_CHARACTOR_L);
    data.flags.select_p = (bool)(ab[IDX_SELECT_P].MS == DIS_CHARACTOR_P);

    for (int i = 0; i < DATA_SET_SIZE; i++)
    {
        // if (ab[i].LS != i) // if packet ID is not matching with packet byte location, mark invalid
        // {
        //     data.error.err_bit.index = true;
        // }
        // if (IDX_TOTAL_START < i && i < IDX_TOTAL_SIZE) // if packet ID is not matching with packet byte location, mark invalid
        // {
        //     if (ab[i].LS != i)
        //         data.error.err_bit.totprice = true;
        // }
        // else if (IDX_UNIT_START < i && i < IDX_UNIT_SIZE) // if packet ID is not matching with packet byte location, mark invalid
        // {
        //     if (ab[i].LS != i)
        //         data.error.err_bit.unitprice = true;
        // }
        // if (IDX_VOLUME_START < i && i < IDX_VOLUME_SIZE) // if packet ID is not matching with packet byte location, mark invalid
        // {
        //     if (ab[i].LS != i)
    ...
    memcpy((void *)dd, (void *)&data, sizeof(cens_6_digit_t));
}
    ...
    while (1)
    {
        if (xQueueReceive(capture_queue, &capture_now, pdMS_TO_TICKS(10)))
        {
            ...
            if (ticks_now - ticks_last[dis_id] > pdMS_TO_TICKS(DIFF_PCKT_SEND_MS) || memcmp(capture_dis[dis_id].ab, capture_now.ab, DATA_SET_SIZE))
            {
                ...
                get_display_data((cens_6_digit_t *)(display_data.ab_data), capture_now.ab);

                if ((((cens_6_digit_t *)(display_data.ab_data))->error.u8int & settings.error_mask.u8int) == 0) // if retain errors are none zero, do not send
                {
                ...
                // else
                // {
                //     const cens_6_digit_t *dis = (cens_6_digit_t *)(display_data.ab_data);
        ...
        if ((xTaskGetTickCount() - ticks_last[TX_ID_DIS1_DATA]) > pdMS_TO_TICKS(SAME_PCKT_SEND_MS))
        {
            ...
            get_display_data((cens_6_digit_t *)(display_data.ab_data), temp.ab);
...
esp_err_t display_censtar_6_digit_init(xQueueHandle *send_queue)
{
    ...
    gpio_isr_handler_add(D1_RCLK, byte_read_end_dis_1, NULL);
    gpio_isr_handler_add(D2_SCLK, bit_read_dis_2, NULL);
    gpio_isr_handler_add(D2_RCLK, byte_read_end_dis_2, NULL);
