 ...
 * Captures both channels' SDATA1 line, one bit per SCLK edge, latched
 * into a codeword_bits-wide codeword on each RCLK edge — no pump-specific
 * decoding, just raw codewords forwarded to main in fixed-size chunked
 * batches (TX_ID_RAW_DIS1_DATA / TX_ID_RAW_DIS2_DATA).
 *
