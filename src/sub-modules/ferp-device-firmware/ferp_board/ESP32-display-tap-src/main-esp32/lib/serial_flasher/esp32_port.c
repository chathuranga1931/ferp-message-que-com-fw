...
void (*reset_trigger_func)(bool level) = NULL;
void (*gpio0_trigger_func)(bool level) = NULL;
...
esp_loader_error_t loader_port_esp32_init(const loader_esp32_config_t *config)
{
    ...
    if(config->reset_trg_fn == NULL || config->gpio0_trg_fn == NULL)
        return ESP_LOADER_ERROR_INVALID_PARAM;
    reset_trigger_func = config->reset_trg_fn;
    gpio0_trigger_func = config->gpio0_trg_fn;

    // if(uart_is_driver_installed(s_uart_port))
...
// {
//     // gpio_reset_pin(s_gpio0_trigger_pin);
//     // gpio_set_pull_mode(s_gpio0_trigger_pin, GPIO_FLOATING);
//     // gpio_set_direction(s_gpio0_trigger_pin, GPIO_MODE_DISABLE);
//     uart_driver_delete(s_uart_port);
// }
...
{
    gpio0_trigger_func(false); // set IO0 LOW
    loader_port_reset_target();
    loader_port_delay_ms(SERIAL_FLASHER_BOOT_HOLD_TIME_MS);
    gpio0_trigger_func(false); // set IO0 HIGH
}

void loader_port_reset_target(void)
{
    reset_trigger_func(true); // set Reset Pin LOW
    loader_port_delay_ms(SERIAL_FLASHER_RESET_HOLD_TIME_MS);
    reset_trigger_func(false); // set Reset Pin HIGH
}
