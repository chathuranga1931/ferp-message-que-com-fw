...
#define SAN6_DEBUG_LOG_EN      LOG_DIS
#define SAN6_WARN_LOG_EN       LOG_EN
#define SAN6_ERROR_LOG_EN      LOG_EN
#define SAN6_INFO_LOG_EN       LOG_EN
...
uint64_t display_data_unit_price_new[NO_NOZZELS];
uint64_t display_data_unit_price[NO_NOZZELS];
uint32_t display_data_unit_price_new_count[NO_NOZZELS];
uint32_t display_data_unit_price_count[NO_NOZZELS];
...
uint32_t pumping_increment_count[NO_NOZZELS];
pumping_state_t pumping_state_value_based[NO_NOZZELS];
pumping_state_t pumping_state_nozzle_based[NO_NOZZELS];
unsigned long pumping_state_ts[NO_NOZZELS] = {0};
unsigned long pumping_state_nozzle_ts[NO_NOZZELS] = {0};
unsigned long pumping_state_pumping_ts[NO_NOZZELS] = {0};


uint64_t corrected_total_price(bool is_updated, uint64_t totalx100){
...
bool sanki6_commit_data(uint8_t idx){
...
void sanki6_update_data(uint8_t idx){
...
bool sanki6_process_data(app_display_data_t * display_data){
    ...
    do{
        if(display_data->unit_pricex100 < 20000){
            ...
            // LOG_MSG_DEBUG(SAN6_DEBUG_LOG_EN, "");
            break;
        }
        ...
        if((ABS(gapx100) > 1) && display_data->total_pricex100 == 0 && display_data->volume_lx1000 > 0){
            // that could be a value with zerors.
            ...
            if(total_price_expectedx100 >= 10000000){
                ...
                if(ABS(gapx100) > 100){
                    ...
                    static uint64_t total_price_lastx100 = 0;
                    if(display_data->total_pricex100 != total_price_lastx100){
                        total_price_lastx100 = display_data->total_pricex100;
                        // logger.log("S6 Total Corrected to = " + String(display_data->total_price) + 
                        //     " Old = " 
                        //     + String(display_value_validated_prev[nozzle_id].total_price));
                    }
                }
            }
...
bool sanki6_process_state_machine(const app_display_data_t * display_data, uint8_t nozzle_id){
    ...
    // LOG_MSG_DEBUG
    // (
    //     "[Value] D [" + std::to_string(nozzle_id) +"] : " + std::to_string(deravative_int)  + " T [" + std::to_string(nozzle_id) +"] : " + 
    //     std::to_string(display_data_validated[nozzle_id].total_pricex100) + 
    //     " -> " + 
    //     std::to_string(display_value_validated_prev[nozzle_id].total_pricex100)
    // );
    ...
    switch (pumping_state_value_based[nozzle_id])
    {
        case Pumping_State_Pumping:
            if(deravative_int == 0){
                if((ts - pumping_state_pumping_ts[nozzle_id]) > 500){
                    ...
                    LOG_MSG_DEBUG(SAN6_INFO_LOG_EN, "[Value] State [%d] : Pumping --> Pumping-Waiting", nozzle_id);
                }
            }
            else if(deravative_int == 1){
                // LOG_MSG_DEBUG(SAN6_DEBUG_LOG_EN, "");
        ...
        case Pumping_State_Pumping_Waiting:
            if(deravative_int == 1){
                ...
                pumping_state_ts[nozzle_id] = ts;
                pumping_state_pumping_ts[nozzle_id] = ts;
                LOG_MSG_DEBUG(SAN6_INFO_LOG_EN, "[Value] State [%d] : Pumping-Waiting --> Pumping ", nozzle_id);
            }
        break;
        ...
        default:        
            if(deravative_int == 1){
                pumping_state_value_based[nozzle_id] = Pumping_State_Pumping;
                pumping_state_ts[nozzle_id] = ts;
                LOG_MSG_DEBUG(SAN6_INFO_LOG_EN, "[Value] State [%d] : Unknown --> Pumping ", nozzle_id);
            }
        break;  
    }
    ...
    switch(pumping_state_nozzle_based[nozzle_id])
    {
        case Pumping_State_Pumping:
            ...
            {                
                ...
                )
                {
                    ...
                    LOG_MSG_DEBUG(SAN6_INFO_LOG_EN, "[Nozzle] State [%d] : Pumping --> Unknown ", nozzle_id);
                }
                else
                {
                    pumping_state_nozzle_based[nozzle_id] = Pumping_State_Stopped_Waiting;
                    LOG_MSG_DEBUG(SAN6_INFO_LOG_EN, "[Nozzle] State [%d] : Pumping --> Stopped-Waiting ", nozzle_id);
                    pumping_state_nozzle_ts[nozzle_id] = ts;
        ...
        case Pumping_State_Stopped_Waiting:
            ...
            )
            {
                ...
                pumping_state_nozzle_ts[nozzle_id] = ts;
                LOG_MSG_DEBUG(SAN6_INFO_LOG_EN, "[Nozzle] State [%d] : Stopped-Waiting --> Stopped ", nozzle_id);
            }            
            else if((pumping_state_nozzle_ts[nozzle_id]) > 700)
            {
                pumping_state_nozzle_based[nozzle_id] = Pumping_State_Stopped;
                pumping_state_nozzle_ts[nozzle_id] = ts;
                LOG_MSG_DEBUG(SAN6_INFO_LOG_EN, "[Nozzle] State [%d] : Stopped-Waiting --> Stopped ", nozzle_id);
        ...
        case Pumping_State_Stopped:
            // sanki6_publish_data(nozzle_id);
            is_pumped_event = true;
            pumping_state_nozzle_based[nozzle_id] = Pumping_State_Unknown;
            pumping_state_nozzle_ts[nozzle_id] = ts;
            LOG_MSG_DEBUG(SAN6_INFO_LOG_EN, "[Nozzle] State [%d] : Stopped --> Unknown ", nozzle_id);
            break;
        case Pumping_State_Unknown:
        default:        
            if(display_data->start_stop)
            {
                ...
                pumping_state_nozzle_ts[nozzle_id] = ts;
                LOG_MSG_DEBUG(SAN6_INFO_LOG_EN, "[Nozzle] State [%d] : Unknown --> Pumping ", nozzle_id);
            }
        break;  
    }
